...
function enviaEmail($destinatario, $mensagem, $forma = 'SES', $replyto = ''){
	...
	do{
		...
		$CI->email->from($smtp['FROM'][0], $smtp['FROM'][1]); // Remetente
		$CI->email->set_alt_message($mensagem['altMensagem']);
		echo "<pre>$email";
		print_r([$config,$smtp,$CI->email]);
...
function userConversa($conversa, $user1){//indica qual o usuário reverso da conversa
	$CI = &get_instance();
...
Senha: <strong>[senha]</strong></p>";
	...
	$ent = str_replace(
			array("[urlanu]", "[parceiro]", "[assinatura]", "[cabecalho]", "[anunciante]", "[usuario]", "[cancelar]", "[edicao]", "[pagamento]", "[conversao]", "[excluir]", "[recet]", "[portal]", "[url]", "[n]", "[/n]", "[i]", "[/i]", "[p]", "[/p]", "[g]", "[/g]", "[gg]", "[/gg]", "[ggg]", "[/ggg]", "[img]", "[/img]"), array($URLcliente, $parceiro, $assinatura, $cabecalho, $anunciante, $anunciante, $cancelar, $edicao, $pagamento, $conversao, $excluir, $recet, $GLOBALS['PORTAL_NOME'], $GLOBALS['PORTAL_URL'], "<strong>", "</strong>", "<em>", "</em>", "<small>", "</small>", "<h3>", "</h3>", "<h2>", "</h2>", "<h1>", "</h1>", "<img border='1' src='", "'>"), $ent
	);
	if(!$html)
		$ent = str_replace("\n", "<br>", $ent);
	$ent = str_replace(
			array("<br><p>", "<br><h1>", "<br><h2>", "<br><h3>", "</p><br>", "</h1><br>", "</h2><br>", "</h3><br>"), array("<p>", "<h1>", "<h2>", "<h3>", "</p>", "</h1>", "</h2>", "</h3>"), $ent
	);
