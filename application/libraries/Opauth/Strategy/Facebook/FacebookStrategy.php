 ...
 * a linha logo assima foi substituida pela de baixo, provavelmente o facebook alterou a forma de retorno dos dados, atens vinha uma string como url, GET, mas de repente passou a retornar um json
 * então foi adicionada a linha abaixo
 */
			...
			if (!empty($results) && !empty($results['access_token'])){
				...
				$this->auth = array(
					...
					'info' => array(
						'name' => $me->name,
						'image' => 'https://graph.facebook.com/'.$me->id.'/picture?width=1000&height=1000',
                                                'email' => $me->email,
                                                'gender' => $me->gender,
                                                'locale' => $me->locale
					),
				...
				$error = array(
					...
					'raw' => $headers,
//					'params'=>$params,//INCLUIDO POR ANDREAS para debugar
//					'response'=>$response, //INCLUIDO POR ANDREAS
//					'results'=>$results//INCLUIDO POR ANDREAS
				);
	...
	private function me($access_token){
		$me = $this->serverGet('https://graph.facebook.com/me', array('access_token' => $access_token,'fields'=>'id,name,email,gender,locale,birthday'), null, $headers);
		if (!empty($me)){
